...
<main>
        <section class="user">
            ...
            <div class="_main-user">
                <div class="title-catalog wow fadeInLeft" data-wow-delay=".3s"><a href="pages/edituser/sua/{{Auth::user()->id}}"><img class="icon" src="./images/icons/add.svg" alt="" srcset=""></a>
                    <div class="name-catalog"><span>A . Event</span><a href="pages/edituser/sua/{{Auth::user()->id}}">
                        @if(!Auth::user() && !isset($loginfb) && !isset($logingg))
                        <h3>Thông tin tài khoản: <strong style="color: #ffd800; !important">{{Auth::user()->name}}</strong></h3></a></div>
                        @endif
                        @if(Auth::user())
                        <h3>Thông tin tài khoản: <strong style="color: #ffd800; !important">{{Auth::user()->name}}</strong></h3></a></div>
                        @endif
                        @if(isset($loginfb))
                        <h3>Thông tin tài khoản: <strong style="color: #ffd800; !important">{{$loginfb->name}}</strong></h3></a></div>
                        @endif
                        @if(isset($logingg))
                        <h3>Thông tin tài khoản: <strong style="color: #ffd800; !important">{{$logingg->name}}</strong></h3></a></div>
                        @endif
                        <div class="thongbao">
                            @if(session('thongbao'))
