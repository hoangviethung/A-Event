...
class Category extends CMS_Controller
{
	...
	{
		$this->id = $_GET['id'];
		$this->data['product'] = $this->M_myweb->set_table('product')->sets(array('category'=>$this->id,'deleted'=>0))->set_orderby('name')->gets();
		if($this->data['product'])
		{
			foreach($this->data['product'] as $key=>$item)
			{
				$category=$this->M_myweb->set_table('category')->sets(array('id'=>$item->category))->get();
				$this->data['product'][$key]->category_name=$category->name;
			}
